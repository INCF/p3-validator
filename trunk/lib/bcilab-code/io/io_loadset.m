...
%
%   Options...    :   --- parameters for subset restriction ---
...
%                                       pairs.
%                     'nofixups'    : disable any fixups of the data (annotations for internal tracking,
%                                     post-hoc data reductions); only for debugging.
%
...
opts = rmfield(allopts,{'filename','types','setname','subject','group','condition','session','comments','markerchannel','arg_direct'});
...
try
    ...
    switch lower(ext)
        case '.set'
            ...
            if ~(isfield(opts,'nofixups') && opts.nofixups) && (~isfield(res,'tracking') || ~isfield(res.tracking,'online_expression'))
...
if (~(isfield(opts,'nofixups') && opts.nofixups) && isempty(res.event)) || allopts.markerchannel.force_processing
    res = set_infer_markers('signal',res,allopts.markerchannel); end

% infer chanlocs fields (e.g. coordinates)
if ~(isfield(opts,'nofixups') && opts.nofixups)    
