...
if ~exist('dipfit_info','var')
    ...
    % determine unique id of this computation...
    global tracking;
    tag = hlp_fingerprint({signal.tracking.expression,{hdm_file,mri_file,chan_file,lookup_labels,confusion_range,brain_atlas,verbose,var_threshold}});
    saveas = [tracking.paths.temp_path filesep 'dipfits' filesep 'dipfit-' num2str(tag) '.mat'];
    if exist(saveas,'file')
        try            
            io_load(saveas);
            disp('Reusing a previous dipfit solution for the same problem...');
        catch
            disp('Found a previous dipfit solution to reuse, but failed to load it...');
        end
    end
    ...
    if ~exist('dipfit_info','var')
        ...
        % fit icaweights
        if verbose
            dipfit_info = do_fitting(signal,mri_file,hdm_file,chan_file,lookup_labels,brain_atlas,confusion_range,var_threshold);
        else
            evalc('dipfit_info = do_fitting(signal,mri_file,hdm_file,chan_file,lookup_labels,brain_atlas,confusion_range,var_threshold);');
        end
        try
            ...
            if isfield(signal.etc,'amica')
                ...
                for m=1:size(signal.etc.amica.W,3)
                    ...
                    if verbose
                        tmp = do_fitting(sig,mri_file,hdm_file,chan_file,lookup_labels,brain_atlas,confusion_range,var_threshold);
                    else
                        evalc('tmp = do_fitting(sig,mri_file,hdm_file,chan_file,lookup_labels,brain_atlas,confusion_range,var_threshold);');
                    end
        ...
        io_save(saveas,'dipfit_info','-makedirs','-attributes','''+w'',''a''');
    end
end
