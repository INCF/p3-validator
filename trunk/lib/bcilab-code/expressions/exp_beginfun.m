...
% early check whether the expression system is disabled
if  ~strcmp(setting,'symbolic') && hlp_resolve('disable_expressions',false,ctx)
...
if isempty(varargin) && strcmp(setting,'symbolic')
    ...
    attribs = {'delayed',1,'add_impure',1,'hold',[],'symbolic',0,'set_online',[],'append_online',[],'argsteps',[],'presteps',[],'poststeps',[],'fingerprint_check',[],'fingerprint_create',[],'memoize',[]};
...
if should_evaluate
    ...
    % begin measuring the evaluation time of the expression (used to determine the best cache
    % location)
    eval_time = tic;
    ...
    context = struct( ...
        ...
        'opts',{opts}, 'eval_time',{eval_time}, 'memoize_id',{memoize_id}, 'exec_ctx',{ctx}, 'enabled',{1});    % misc fields
