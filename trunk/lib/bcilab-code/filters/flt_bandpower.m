...
declare_properties('name', 'BandPower', 'cannot_follow', 'set_makepos', 'follows', 'flt_ica', 'precedes', 'flt_iir', 'independent_channels', false, 'independent_trials',true);
...
if all_channels  % apply filters to all channels
    ...
    for n_filter = 1:n_bands(1)
        ...
        signal_filtered{n_filter} = transpose(log(filter(ones(round(smoothing*signal.srate),1),round(smoothing*signal.srate),transpose(filter(state{n_filter},double(signal.data),2).^2))));
    end
    ...
    for n_channel = 1:length(bands)
        for n_filter = 1:n_bands(n_channel)
            ...
            % apply the filter, smooth with moving average, and take the logarithm
            signal_filtered{n_channel}{n_filter} = transpose(log(filter(ones(round(smoothing*signal.srate),1),round(smoothing*signal.srate),transpose(filter(state{n_channel}{n_filter},double(signal.data),2).^2))));
