...
arg_define(varargin,...
    ...
    arg({'mttapers','Tapers'},[],[],'Number of tapers. Should be an integer smaller than 2*TimeBandwith; default 2*TimeBandwidth-1'), ...
    arg({'mtpadding','Padding'},0,[],'FFT padding factor. Controls the oversampling of the spectrum; 0 is the next largest power of two, 1 is 2x as much, etc.'), ...
...
for t = 1:length(tmp)
    ...
    [spec,F] = mtspectrumpb(signal.data(:,:,t)',struct('tapers',[2*mtbandwidth mttapers],'pad',mtpadding,'Fs',signal.srate));
