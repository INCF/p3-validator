...
%   [1] H. Shen, S. Jegelka and A. Gretton. "Fast Kernel ICA using an approximate Newton method."
%       AISTATS 2007.
%
% See also:
...
arg_define([0 2],varargin, ...
    ...
    arg_subswitch({'variant','Variant'},'amica',{ ...
    ...
    arg({'dodebug','DebugMode','debug'},false,[],'Debug mode. Stops and waits for user input in case of an exception.'), ...
    ...
    if doforce || ~(isfield(signal,'icaweights') && ~isempty(signal.icaweights))
        ...
        switch variant.arg_selection %#ok<*NODEF>
            ...
            case 'kernelica'
                ...
                pre.icaweights = kernel_ica_options(pre.data,arglist{:});
                pre.icasphere = eye(size(pre.data,1));
            case 'fastkernelica'
                % fast KernelICA is an euphemism
                C = size(pre.data,1);
                pre.icasphere = 2.0*inv(sqrtm(cov(pre.data'))); %#ok<MINV>
                guess = eye(C);
                for k=1:variant.restarts
                    tmpdata = (pre.icasphere * pre.data);
                    [Ws{k}, dummy, HSIC] = fastkica(tmpdata, guess, variant.maxiter, variant.psigma, variant.thresh); %#ok<ASGLU>
                    guess = eye(C) + randn(C);
                    HSICs(k) = min(HSIC);
                end
        ...
        try
            ...
            % also normalize weights for consistency
