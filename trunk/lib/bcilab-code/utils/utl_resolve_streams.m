...
if ~strcmp(phead,'rawdata')
    ...
    elseif strcmp(phead,'flt_seltypes')
        ...
        if isempty(type_subset)
            ...
            % the channel selection is the argument following that (it is a name-value pair)
            ti = find(strcmp(pip.parts,'types'),1,'last');
    ...
    for s = 1:length(streams)
        ...
        satisfies_channels(s) = isempty(fast_setdiff(needed_channels,streams{s}.labels)); 
        satisfies_types(s) = length(pip.parts{1}) == length(streams{s}.labels) && isequal(sort(needed_types),streams{s}.types);
    end
...
elseif all(isfield(stream,{'head','parts'}))
    ...
    switch(shead)
        ...
        case 'flt_seltypes'
            ...
            [dummy,ia] = intersect({subprops.chanlocs.type},selection); %#ok<ASGLU>
