function utl_memoize_commit(obj,id)
% Commit an object to a memory location.
% utl_memoize_commit(Object,Location)
%
% In:
...
elseif id(1) == filesep
    ...
    try
        ...
        try
            write_time_estimate = objinfo.bytes * regress([location.writestats.time]',[location.writestats.size]');
            read_time_estimate = objinfo.bytes * regress([location.readstats.time]',[location.readstats.size]');
            time_without_caching = obj{1}.tracking.computation_time * tracking.cache.reuses;
        ...
        try 
            if location.space_checker.getFreeSpace - objinfo.bytes < location.free                
