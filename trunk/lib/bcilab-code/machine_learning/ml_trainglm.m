...
args = arg_define([0 2],varargin, ...
    ...
    arg({'outerZinit','InitialUpperBound'},0.05,[],'Initial upper bound.'),...
    arg({'outerGainit','InitialVariationalParam'},1,[],'Initial variational parameter.'),...
    ...
    arg({'innerEPeta','InnerEPEta'},1,[],'Fractional EP parameter. Parameter used for fractional expectation-propagation (only works with Gauss and Laplace potential functions).'), ...
...
% run inference
[uinf,ga,b,z,nlZ,Q,T] = dli(X,y,lambdas,B,pot,tau,rmfield(args,{'trials','targets','lambdas','ptype','scaling','setupfcn','shape','doinspect'}),G); %#ok<ASGLU>
