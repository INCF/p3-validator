...
opts=arg_define([0 3],varargin, ...
    ...
    arg({'params','KernelParams'}, [], [], 'Vector of kernel parameters. If empty, chosen according to the author''s recommendations (Francis Bach).','shape','row','cat','Core Parameters'), ...
    arg({'mingap','Precision'}, 1e-3, [], 'Desired precision. Expressed as the duality gap.','cat','Core Parameters'), ...
    arg({'gapeta','GapETA'},1e-3,[],'Smoothing degree of the reduced problem.','cat','Core Parameters'), ...
    ...
    ...
    arg({'data_normaliation','DataNormalization'},'center',{'center','scale'},'Data normalization. Very important if the data is not naturally normalized. Note: scale both centers and standardizes.','cat','Miscellaneous'), ...
    arg({'kernel_normaliation','KernelNormalization'},'scale-root',{'center','scale','scale-root'},'Normalization in the kernel space.','cat','Miscellaneous'), ...
    arg({'alphaparam','Alpha','alpha'},[],[],'Dual parameter. See the HAL 00413473 TechReport.','cat','Miscellaneous'), ...
    arg({'eta','Eta'},[],[],'Kernel Weights. See the HAL 00413473 TechReport.','cat','Miscellaneous'), ...
...
if length(lambdas)>1
    ...
    [model.outputs,model.model] = hkl(trials,targets,lambdas,loss,kernel,params,args{:});
    model.bestlambda = 1;
end
