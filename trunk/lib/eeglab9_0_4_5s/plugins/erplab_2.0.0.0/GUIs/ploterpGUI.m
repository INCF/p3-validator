...
if ~isempty(binArray) && ~isempty(chanArray)
      ...
      timeticks = str2num(get(handles.edit_time_ticks, 'String')); % XL
      ...
      if ~isempty(timeticks)
            if istopo
                  ...
                  %if max(timeticks)>max(timew)  ||  min(timeticks)<min(timew)
                  %      title = 'ERPLAB: ploterpGUI inputs';
                  %      question = ['Some Time Tick values are larger than Time scale!\n'...
                  %            'Do you want to continue anyway?'];
                  %      button = askquest(sprintf(question), title);
                  %
                  %      if ~strcmpi(button,'yes')
      ...
      yticks = str2num(get(handles.edit_yticks, 'String')); % YL
      
      if size(yscale,1)~=1 || size(yscale,2)~=2
            msgboxText =  'Wrong scale format!';
            title = 'ERPLAB: ploterpGUI inputs';
            errorfound(msgboxText, title);
            plotset = evalin('base', 'plotset');
            plotset.ptime = [];
      ...
      if ~isempty(yticks)
            if istopo
                  ...
                  %if max(yticks)>max(yscale)  ||  min(yticks)<min(yscale)
                  %      title = 'ERPLAB: ploterpGUI inputs';
                  %      question = ['Some Y Tick values are larger than Y scale!\n'...
                  %            'Do you want to continue anyway?'];
                  %      button = askquest(sprintf(question), title);
                  %
                  %      if ~strcmpi(button,'yes')
      ...
      %else
      minorticks = [get(handles.checkbox_minorticksX, 'Value') get(handles.checkbox_minorticksY, 'Value')];
      ...
      plotset.ptime.xscale         = [timew timeticks]; % 2 or more values...
      plotset.ptime.yscale         = [yscale yticks];  % 2 or more values...
...
set(handles.figure1,'Name', ['ERPLAB ' version '   -   ERP Plotting GUI  -  ' ERP.erpname],...
...
if ~isempty(ptime)
      ...
      multick = 10*round(((xxs2-xxs1)/6)/10); % ~6 time points total

      xxstick1 = (round(xxs1/100)+ 1)*100;
      xxstick2 = (round(xxs2/100)+ 1)*100;
      xtickarray = xxstick1:multick:xxstick2;
      xtickarray = xtickarray(xtickarray<=xxs2 & xtickarray>=xxs1);      
      
      xscale     = [xxs1 xxs2 xtickarray];
      yscale     = [-10 10 -10:5:10];
      linewidth  = 1;
      isiy       = 0;
      fschan     = 10;
      fslege     = 10;
      ...
      % Default Ticks
      %
      set(handles.checkbox_default_ticks_time, 'Value', 1)
      set(handles.checkbox_default_ticks_Y, 'Value', 1)
      set(handles.edit_time_ticks,'Enable', 'off')
      set(handles.edit_yticks,'Enable', 'off')      
...
if istopo
      ...
      set(handles.checkbox_minorticksX,'Enable', 'off')
      set(handles.checkbox_minorticksY,'Enable', 'off')
else
      ...
      set(handles.checkbox_minorticksX,'Enable', 'on')
      set(handles.checkbox_minorticksY,'Enable', 'on')
end
...
% X scale & ticks
%
xl1 = sprintf('%.1f %.1f', xscale(1), xscale(2));
if length(xscale)>2
      xl2 = sprintf('%s', vect2colon(xscale(3:end),'Delimiter','off'));
else
      xl2 = '';
end

set(handles.edit_time_range,'String', xl1)
set(handles.edit_time_ticks,'String', xl2)

%
% X, Y MinorTicks
%
set(handles.checkbox_minorticksX,'Value', minorticks(1))
set(handles.checkbox_minorticksY,'Value', minorticks(2))
...
%  Y scale
%
yscalestr1 = sprintf('%.1f %.1f', yscale(1), yscale(2));
if length(yscale)>2
      yscalestr2 = sprintf('%s',vect2colon(yscale(3:end),'Delimiter','off'));
else
      yscalestr2 = ''; % sprintf('%.1f %.1f', yscale);
end

set(handles.edit_yscale, 'String', yscalestr1)
set(handles.edit_yticks, 'String', yscalestr2)
...
%
handles.mxscale =  xscale;
handles.myscale =  yscale;
...
if get(hObject, 'Value')
      ...
      %
      if length(mxscale)>2
            trangestr = sprintf('%.1f %.1f  %s', mxscale(1), mxscale(2), vect2colon(mxscale(3:end),'Delimiter','off'));
      else
            trangestr = sprintf('%.1f %.1f', mxscale);
      end
      set(handles.edit_time_range,'String', trangestr)
      
      if length(myscale)>2
            yscalestr = sprintf('%.1f %.1f  %s', myscale(1), myscale(2), vect2colon(myscale(3:end),'Delimiter','off'));
      else
            yscalestr = sprintf('%.1f %.1f', myscale);
      end
...
%--------------------------------------------------------------------------
function checkbox_minorticksY_Callback(hObject, eventdata, handles)

%--------------------------------------------------------------------------
function checkbox_minorticksX_Callback(hObject, eventdata, handles)

%--------------------------------------------------------------------------
function edit_time_ticks_Callback(hObject, eventdata, handles)

%--------------------------------------------------------------------------
function edit_time_ticks_CreateFcn(hObject, eventdata, handles)
...
%--------------------------------------------------------------------------
function edit_yticks_Callback(hObject, eventdata, handles)

%--------------------------------------------------------------------------
function edit_yticks_CreateFcn(hObject, eventdata, handles)
...
%--------------------------------------------------------------------------
function checkbox_default_ticks_Y_Callback(hObject, eventdata, handles)

if get(hObject,'Value')
      ytickarray = -10:5:10;
      set(handles.edit_yticks,'String', vect2colon(ytickarray,'Delimiter','off'))
      set(handles.edit_yticks,'Enable', 'off')      
else
      try
            plotset = evalin('base', 'plotset');
            yscale  = plotset.ptime.yscale;
            if length(yscale)>2
                  set(handles.edit_yticks,'String', vect2colon(yscale(3:end),'Delimiter','off'))
            end
            
      catch
      end
      set(handles.edit_yticks,'Enable', 'on')
end

%--------------------------------------------------------------------------
function checkbox_default_ticks_time_Callback(hObject, eventdata, handles)
if get(hObject,'Value')
      ERP = handles.ERP;
      %
      % ticks for time
      ...
      xtickarray = xxstick1:multick:xxstick2;
      xtickarray = xtickarray(xtickarray<=xxs2 & xtickarray>=xxs1);
      set(handles.edit_time_ticks,'String', vect2colon(xtickarray,'Delimiter','off'))
      set(handles.edit_time_ticks,'Enable', 'off')
else
      try
            plotset = evalin('base', 'plotset');
            xscale  = plotset.ptime.xscale;
            if length(xscale)>2
                  set(handles.edit_time_ticks,'String', vect2colon(xscale(3:end),'Delimiter','off'))
            end
            
      catch
      end
      set(handles.edit_time_ticks,'Enable', 'on')
end
