...
if isempty(listname) && get(handles.checkbox_sendfile2history,'Value')==1
      ...
      if strcmpi(button,'Save and run')
            ...
            listname = fullname;
            handles.output = {listname, 0}; % sent filenam string)
      elseif strcmpi(button,'Run without saving')
            handles.output = {cellstr(formulalist), 0}; % sent like a cell string (with formulas)
...
elseif ~isempty(listname) && get(handles.checkbox_sendfile2history,'Value')==1
      handles.output = {listname, 0}; % sent filename string
...
if get(hObject,'Value')
      set(handles.button_no_recu,'Value',0)
      testsyntaxtype(hObject, eventdata, handles, 'recu')
%       handles = editorbackup(hObject, eventdata, handles);
      % Update handles structure
      guidata(hObject, handles);
else
...
if get(hObject,'Value')
      set(handles.button_recursive,'Value',0)
      testsyntaxtype(hObject, eventdata, handles, 'norecu')
%       handles = editorbackup(hObject, eventdata, handles);
      % Update handles structure
      guidata(hObject, handles);
else
...
%--------------------------------------------------------------------------
function testsyntaxtype(hObject, eventdata, handles, whocall)
...
for t=1:nformulas
      ...
      if isempty(pleft) && strcmpi(whocall,'norecu')
            if ask4fix
                  BackERPLABcolor = [1 0.9 0.3];    % yellow
                  question = ['For non recursive mode, left side of equation must be define as a new channel.'...
                        'For instance, nchan1 = ...\n\n'...
      ...
      elseif ~isempty(pleft) && strcmpi(whocall,'recu')
            if ask4fix
                  BackERPLABcolor = [1 0.9 0.3];    % yellow
                  question = ['For recursive mode, left side of equation cannot be define as a new channel.'...
                        'For instance, you must write chan1 = ...\n\n'...
                        'Do you want that ERPLAB corrects the syntax for you?'];
                  title = 'WARNING: Syntax is not proper for recursive mode';
                  oldcolor = get(0,'DefaultUicontrolBackgroundColor');
                  set(0,'DefaultUicontrolBackgroundColor',BackERPLABcolor)
                  button = questdlg(question, title,'Cancel','No', 'Yes','Yes');
                  set(0,'DefaultUicontrolBackgroundColor',oldcolor)
      ...
      if wantfix
            ...
            if strcmpi(whocall,'recu')
                  formulaArray{t} = sprintf('%s = %s', strtrim(regexprep(parts{t}{1}, 'n','','ignorecase')), strtrim(parts{t}{2}));
            else
                  formulaArray{t} = sprintf('%s = %s', strtrim(regexprep(parts{t}{1}, '[^n]*ch','nch','ignorecase')), strtrim(parts{t}{2}));
            end
...
if ~isempty(chan2del)
      ...
      handles.output = {'deletechan', chan2del}; % sent like a cell string (with formulas)
