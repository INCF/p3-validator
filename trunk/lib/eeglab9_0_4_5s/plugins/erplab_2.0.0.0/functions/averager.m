...
for i = 1:nepoch
      if length(EEG.epoch(i).eventlatency) == 1
            numbin = EEG.epoch(i).eventbini; % might be more than one.
            ...
            if ~isempty(find(tf==1, 1))
                  ...
                  elseif ismember(-1, enableALL)
                        ...
                        if ~iscell(artcrite)  
                              ...
                              %
                              % Specified epoch indices
                              %
                              specepoch = ismember(i, cell2mat(artcrite)); % Is this epoch one of the specified ones?
