...
% end
% if size(chin,1)>1
%         error('ERPLAB says: error, chreref works with row-array inputs')
% end
% 
...
%         error('ERPLAB says: error at chreref. Some specified channels do not exist!')
% end
% 
% ERPLAB.bindata(chanarray,:,:)=[];
% ERPLAB.nchan = size(ERPLAB.bindata, 1);
% 
% if isfield(ERPLAB.chanlocs, 'labels')
%         if ~isempty([ERPLAB.chanlocs.labels])
%                 labaux = {ERPLAB.chanlocs.labels};
%                 [labaux{chanarray}] = deal([]);
%                 indxl    = ~cellfun(@isempty, labaux);
%                 labelout = labaux(indxl);
%                 ERPLAB.chanlocs = [];
%                 [ERPLAB.chanlocs(1:ERPLAB.nchan).labels] = labelout{:};
%         end
% end
