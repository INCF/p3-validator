...
% end
% if nargin<2
%       error('ERPLAB says: uncorrar.m needs at least 2 inputs.')
% end
% if nargin<5
...
%                         error('error: cutoff must have 2 different values. Except for [0 0] or [Inf Inf]. See help uncorrar')
%                   end
...
% %
% if length(EEG.event)<1
%       fprintf('\nuncorrar.m.m did not found remaining event codes.\n')
%       return
% end
% if ischar(EEG.event(1).type)
%       codebound = {EEG.event.type}; %strings
...
%       fprintf('WARNING: uncorrar.m will be applied over the full range of data.\n\n');
% end
