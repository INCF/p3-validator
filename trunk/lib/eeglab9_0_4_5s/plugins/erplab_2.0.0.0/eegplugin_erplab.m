...
if ~exist('pop_binlister.m','file')
        addpath(p);
        addpath([p 'erplab_help'], [p 'erplab_Temp'], [p 'functions'], [p 'GUIs'], [p 'images'], [p 'pop_functions']); % Thanks to Grega Repovs
end
...
save(fullfile(p,'memoryerp.erpm'), 'erplabrel','erplabver', 'ColorB', 'ColorF'); % saves erplab version
%---------------------------------------------------------------------------------------------------
...
%comIMerpERPSS = ['[ERP ERPCOM] = pop_asc2erp;' '[ERP ALLERPCOM] = erphistory(ERP, ALLERPCOM, ERPCOM);'];
%comIMerpTEXT  = ['[ERP ERPCOM] = pop_utext2erp;' '[ERP ALLERPCOM] = erphistory(ERP, ALLERPCOM, ERPCOM);'];
%comIMerpNEURO = ['[ERP ERPCOM] = pop_neuro2erp;' '[ERP ALLERPCOM] = erphistory(ERP, ALLERPCOM, ERPCOM);'];
% load ERP
...
uimenu( mAR, 'Label', 'Blocking & Flat line'  , 'CallBack', comAR8);
...
uimenu( submenu, 'Label',  'Append ERPsets'  , 'CallBack', comAPP);%, 'separator','on');
uimenu( submenu, 'Label', '<html>Plot <b>ERP</b> Waveforms'   , 'CallBack', comPLOT);
uimenu( submenu, 'Label', '<html>Plot <b>ERP</b> Scalp Maps'   , 'CallBack', comSCALP);
uimenu( submenu, 'Label', 'Export plotted figure to PDF/EPS', 'CallBack', comEXPPDF, 'separator', 'on');
% export ERP
mEXERP = uimenu( submenu,    'Label', '<html>Export <b>ERP</b>'  , 'tag','Exerp','separator','on');
uimenu( mEXERP, 'Label', '<html>Export <b>ERP</b> to Text (readable by ERPSS)', 'CallBack', comEXPAVG);
uimenu( mEXERP, 'Label', '<html>Export <b>ERP</b> to Text (universal)', 'CallBack', comEXPUNI, 'separator', 'on');
% Import ERP
uimenu( submenu,    'Label', '<html>Import <b>ERP</b>'  , 'CallBack', comIMPERP,'separator','on');
% mIMERP = uimenu( submenu,    'Label', '<html>Import <b>ERP</b>'  , 'tag','Imerp','separator','on');
% uimenu( mIMERP, 'Label', '<html>Import <b>ERP</b> from ERPSS (text format)', 'CallBack', comIMerpERPSS);
% uimenu( mIMERP, 'Label', '<html>Import <b>ERP</b> from universal text format', 'CallBack', comIMerpTEXT, 'separator', 'on');
% uimenu( mIMERP, 'Label', '<html>Import <b>ERP</b> from Neuroscan avg file', 'CallBack', comIMerpNEURO, 'separator', 'on');
%Load ERPset(s)
...
uimenu( mUTI, 'Label', '<html><FONT color="blue" >Find more here! <FONT color="black">(for scripting)', 'CallBack', 'web(''http://www.erpinfo.org/erplab/erplab-documentation/utilities/view'',''-browser'');', 'separator', 'on');
...
uimenu( submenu, 'Label', '<html><FONT color="blue" >E-mail Us', 'CallBack', 'web(''mailto:erplabtoolbox@gmail.com'');', 'separator', 'on');
