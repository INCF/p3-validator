...
if isempty(EEG.data)
      ...
      msgboxText{2} =  'ERROR: pop_artblink2() cannot read an empty dataset!';
...
%       msgboxText{1} =  'Permission denied:';
%       msgboxText{2} =  'pop_artblink2 has been tested for epoched data only';
%       title = 'ERPLAB: pop_artblink2 Permission';
%       errorfound(msgboxText, title);
%       return
% end
if ~isempty(EEG.epoch)
...
%       testwindow = vect2colon(testwindow);
% end
% if ~ischar(evcode)
%       evcode = vect2colon(evcode);
% end
...
%(EEG, evcode, twin, bwidth, ccovth, chan, flag)
% com = sprintf( '%s = pop_artblink2( %s, %s, %s, %s, %s, %s, %s);', ...
%       inputname(1), inputname(1), evcode, testwindow, num2str(blinkwidth), ...
