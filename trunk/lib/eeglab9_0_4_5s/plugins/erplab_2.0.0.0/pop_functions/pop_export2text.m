...
% FORMAT   :
% 
% >> ERP = pop_export2text(ERP);      %window will pop up
% >> ERP = pop_export2text(ERP, filename, 'key', 'val', ... );
% 
% EXAMPLE  :
% 
% >> ERP = pop_export2text(ERP);      %window will pop up
% >> ERP = pop_export2text( ERP, '/Users/etfoo/Documents/MATLAB/test.txt',
% [ 1 2],'time','on','timeunit',1,'elec','on','transpose','off',
% 'precision',4);                                                                                                                                                                                                                                                                                                                                                                                             
% 
% INPUTS   :
% 
...
% 'timeunit' 	- [float] time unit relative to seconds. Default: 1E-3
%               = msec.
% 'elec'        - ['on'|'off'] include electrode names or component 
%               numbers. Default 'on'.
% 'transpose'	- ['on'|'off'] 'off'-> electrode data = rows; 'on' -> 
%               electrode data = columns. Default 'off'.
...
%
%
% GUI: export2textGUI.m ; SUBROUTINE: --
%
%
...
for ibin=1:nbin
      ...
      if strcmpi(p.Results.time, 'on');            
            fprintf('bin #%g\n', ibin);
            time_val = (ERP.times/1000)/p.Results.timeunit; %Nov 2010
      ...
      if strcmpi(p.Results.transpose, 'off') % no transpose
            ...
            for index = 1:size(data,1)
                  ...
                  if strcmpi(p.Results.time, 'on') % show time values
                        tmpind = index-1;
                  else
                        tmpind = index;
                  end
